...
<title>Employee Form</title>
	<script >
		function updateSalary(index , salary) {
			var newSalary = prompt('請輸入薪資' , salary);
			  window.location.href="${pageContext.request.contextPath}/mvc/employee/"+index+"/salary?salary="+newSalary;			  
		}	
	</script>
...
<body style="padding:20px">
	...
	<spform:form modelAttribute="employee" 
				 ...
				 action="${pageContext.request.contextPath}/mvc/employee/">
		<fieldset>
	...
	<table class="pure-table pure-table-bordered">
		...
		<tbody>
			...
			<tr>
				<td>${ status.index }</td>
				<td>${ employee.age}</td>
				<td title="按我一下可以修改" style="color: blue;cursor: pointer;"
					onclick="updateSalary(${status.index }, ${employee.salary })">${employee.salary }</td>
